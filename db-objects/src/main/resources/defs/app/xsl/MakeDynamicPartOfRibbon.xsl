	...
	<xsl:template name="MakeDynRibbon">
		...
		<xsl:result-document href="{concat($_tmpDir, 'dynamicPartOfRibbon.xml')}" format="format" validation="strict">
			...
			<xsl:if test="count($res/isc:MenuPathes/isc:MenuPath) &gt; 0 or $existsExt1 = true()">
				<isc:Members xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://simpleSys.ru/xml/library/ISC" xsi:schemaLocation="http://simpleSys.ru/xml/library/ISC  http://toucan.simplesys.lan/xml/xsd/schemaISC.xsd">

...
create or replace package body Record_Doc is
  procedure MainRecOrdDoc(source_srcName       varchar2,
    ...
    edit_id number;
  begin
    ord_doc := new ordsys.orddoc(source        => new
                                                  ordsys.ordsource(localData   => source_localData,
                                                                   srcType     => source_srcType,
                                                                   srcLocation => source_srcLocation,
                                                                   srcName     => source_srcName,
                                                                   updateTime  => source_updateTime,
                                                                   local       => source_local),
                                 format        => orddoc_format,
                                 mimeType      => orddoc_mimeType,
                                 contentLength => orddoc_contentLength,
                                 comments      => orddoc_comments);
  
    update arx_attatch set attfile = ord_doc where id = fid;
  end;
  ...
  exception
    when NO_DATA_FOUND then
      insert into arx_attatch_blk (id) values (fid);

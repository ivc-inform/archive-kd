	...
	getClientOnlyResponse: `function (request, serverData) {
        ...
        if (!serverData || isc.isA.String(serverData)) {
            if (isc.isA.String(serverData)) {
                ...
                //>DEBUG
                this.logInfo(
                    this.ID + " datasource: testData property and " + this.ID +
        ...
        switch (operationType) {
            ...
            case "delete": // old name
                if (this.isMissingPrimaryKeys(request.data)) {
                    ...
                    //response.data = "clientOnly remove operation failed: missing primaryKey " + "value(s): " + this.getMissingPrimaryKeys(request.data);
                    //response.status = -1;

                    response.status = 0;
	                response.data = strEmpty
                } else {
                    var serverRecordIndex = this.findByKeys(request.data, serverData);
                    if (serverRecordIndex == -1) {
                        this.logWarn("clientOnly remove operation: Unable to find record matching criteria:" + this.echo(request.data));
	                    response.data = strEmpty
	                    response.status = 0;
                        //response.data = "clientOnly remove operation failed: unable to find matching " + "record"
                        //response.status = -1;
                    } else {
            ...
            case "insert": // old name
                ...
                if (serverRecordIndex != -1) {
                    this.logWarn(
                        "clientOnly add operation: Duplicate key: " +
                        isc.echoAll(serverRecord) + this.getStackTrace()
                    );
                    response.data = "clientOnly add operation failed for DataSource " + this.ID + ": Duplicate key in record " + isc.echoAll(serverRecord) + "<br><br>" + this.getStackTrace();
                    response.status = -1;
                } else {
            ...
            case "update":
                if (this.isMissingPrimaryKeys(request.data)) {
                    this.logWarn(
                        "clientOnly update operation: Missing primaryKey values: " +
                        this.getMissingPrimaryKeys(request.data)
                    );
                    response.data = "clientOnly update operation failed for DataSource " + this.ID + ": missing primaryKey values(s) " + this.getMissingPrimaryKeys(request.data);
                    response.status = -1;
                } else {
                    // find the server record
                    var serverRecordIndex = this.findByKeys(request.data, serverData);
                    if (serverRecordIndex == -1) {
                        this.logWarn("clientOnly update operation: Unable to find record matching criteria:" + this.echo(request.data));
                        //response.data = "clientOnly update operation failed: unable to find matching " + "record. Did you supply all primaryKeys?"
                        //response.status = -1;
